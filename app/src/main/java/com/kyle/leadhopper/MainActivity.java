...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        s.setBuiltInZoomControls(false); s.setDisplayZoomControls(false); s.setMediaPlaybackRequiresUserGesture(true);
        webView.addJavascriptInterface(new AndroidBridge(), "AndroidBridge");
        webView.setWebViewClient(new WebViewClient() {
            @Override public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest req) {
                Uri u = req.getUrl();
                if ("tel".equalsIgnoreCase(u.getScheme())) { try { startActivity(new Intent(Intent.ACTION_DIAL, u)); } catch (Exception e) { Toast.makeText(MainActivity.this, "Could not open dialer.", Toast.LENGTH_SHORT).show(); } return true; }
                if ("mailto".equalsIgnoreCase(u.getScheme())) { try { startActivity(new Intent(Intent.ACTION_SENDTO, u)); } catch (Exception e) { Toast.makeText(MainActivity.this, "Could not open email app.", Toast.LENGTH_SHORT).show(); } return true; }
                return false;
            }
        });
